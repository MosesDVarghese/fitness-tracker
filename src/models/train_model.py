...
for i, feature_set in enumerate(possible_feature_sets):
    print(f"Training with {feature_names[i]}")
    
    # Check for missing features in the feature set
    missing_features = [f for f in feature_set if f not in x_train.columns]
    
    if missing_features:
        print(f"Missing features in {feature_names[i]}: {missing_features}")
        continue
    ...
    new_scores = pd.DataFrame(
        {
            "model": ["RF"],
            "feature_set": feature_names[i],
            "accuracy": [performance_test_rf],
        }
    )
...
for i, f in zip(range(len(possible_feature_sets)), feature_names):
    print("Feature set:", i)
    print("Available features:", x_train.columns)
    print("Requested features:", possible_feature_sets[i])
    
    # Check if all requested features are available in the DataFrame columns
    missing_features = [f for f in possible_feature_sets[i] if f not in x_train.columns]
...
missing_features = [f for f in feature_set_4 if f not in x_train.columns]

if missing_features:
    print(f"Missing features in feature_set_4: {missing_features}")
else:
    ...
    print(f"Accuracy with feature_set_4: {accuracy}")
